...
//        navBarHidden = self.bl_naviBarHidden
        if navBarHidden == true {
            self.marginTop = GK_SAFEAREA_TOP
        } else {
            self.marginTop = NAVBAR_HEIGHT
        }
...
//        self.bl_naviBarHidden = true
        ...
        navBar.frame = CGRect(x: 0, y: 0, width: self.view.bounds.size.width, height: NAVBAR_HEIGHT)
    }
    ...
    func setupWebview() {
        ...
        webView?.snp.remakeConstraints({ (make) in
            ...
            make.height.equalTo(SCREEN_HEIGHT - marginTop)
        })
        ...
        webView!.scrollView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: GK_SAFEAREA_BTM, right: 0)
...
//        self.showFrameworkLoading(offsetY: NAVBAR_HEIGHT)
        guard url == nil else {
            if let url = url {
                let request = URLRequest(url: url)
                webView?.load(request)

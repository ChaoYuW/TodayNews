...
class YDMomentTableViewCell: UITableViewCell {
    ...
    //标题
    lazy var textView: ECTextView = {
        let textView = ECTextView()
    ...
    // 代理
    weak var delegate: ECTableViewCellDelegate?
    ...
    // MARK: ReloadData
    func configureCell(model: ECModel, layout: ECLayout?) -> Void {
        let url = URL(string:model.headPic)
        let placeholderImage = UIImage(named: "ListSmalldefaultImg")!
